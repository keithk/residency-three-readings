...
export function totalSteps(scenario: Scenario): number {
  ...
  return 1 /* facts */ + 1 /* llm-answer */ + walk + 3 /* personas */ + 1 /* reveal */;
}

export function stepKind(scenario: Scenario, step: number): StepKind {
  if (step <= 0) return { kind: "facts" };
  if (step === 1) return { kind: "llm-answer" };
  const walk = scenario.walkthrough?.length ?? 0;
  if (step <= walk + 1) return { kind: "walkthrough", index: step - 2 };
  if (step === walk + 2) return { kind: "persona", personaId: "legal" };
  if (step === walk + 3) return { kind: "persona", personaId: "worker" };
  if (step === walk + 4) return { kind: "persona", personaId: "director" };
  return { kind: "reveal" };
}
...
export function stepLabels(scenario: Scenario, step: number): StepLabels {
  ...
  if (sk.kind === "facts") {
    return { short: "Read the facts", next: "See the model's first answer" };
  }
  if (sk.kind === "llm-answer") {
    return { short: "Model's answer", next: "Begin walkthrough" };
  }

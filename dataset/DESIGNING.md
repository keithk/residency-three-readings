...
The methodology's load-bearing decision. The personas are not the prose around the cases; they are the lenses that produce the disagreement. If the personas are wrong, the dataset is wrong, regardless of how well the cases are written.
...
The validator script at `scripts/validate-dataset.ts` is coupled to the SNAP dataset's TypeScript source shape. Adapters working from generated markdown will need to either fork the script or reimplement the invariant checks in their own toolchain. The checks themselves are mechanical; reimplementation is not deep work, it just isn't done for you. A reasonable adapter path: parse the YAML frontmatter with `gray-matter` (already a dependency), then write seven small assertions, one per invariant.

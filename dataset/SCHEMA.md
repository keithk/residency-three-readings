...
This document describes the YAML frontmatter shape used by scenario files in `dataset/scenarios/`. The canonical sources live in `src/data/scenarios/*.ts`; the markdown files are generated from those by `scripts/build-dataset.ts`. The schema below describes the generated frontmatter shape, which is also what the reference scorer at `dataset/scorer/` parses. For the methodology behind the design choices, see `DESIGNING.md`. For running the dataset against a model, see `EVALUATING.md`.
...
- `min_named_readings`: minimum number of distinct readings the model should produce (currently 3).
- `min_grounding_phrases_per_reading`: minimum phrase citations per reading for the grounding eval (currently 1).
...
The validator at `scripts/validate-dataset.ts` runs against the canonical TypeScript sources (not the generated markdown) and enforces the following:
...
The validator runs automatically as part of `bun run build` and refuses to build the site if any invariant fails. To run it on its own:

```bash
bun run dataset:validate
```

The validator is currently coupled to the SNAP dataset's TypeScript source shape. Adapters working from generated markdown will need to either reproduce these invariant checks in their own toolchain or fork the validator.
...
The reference scorer at `dataset/scorer/` accepts arbitrary persona slugs without modification. The validator script (`scripts/validate-dataset.ts`) is currently coupled to the SNAP dataset's TypeScript shape; adapters will need to either reproduce its invariant checks in their own toolchain or fork the script. For the methodology side of adaptation (which fields actually carry the weight, how to pick personas, how to construct the calibration baseline), see `DESIGNING.md`.

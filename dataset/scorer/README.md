...
- **Short citations.** Citations shorter than 8 normalized characters are ignored during phrase matching. A model citing the word `"her"` should not substring-match every phrase in the case that contains "her". A reading whose citations are all below threshold is flagged as `unanchored`.
...
19 tests cover the three evals, the wrapper, and the edge cases above.

## license

The scorer code (`dataset/scorer/`) is MIT-licensed. The surrounding dataset content (`dataset/scenarios/`, prose docs) is CC BY 4.0. See the repository root LICENSE files for details.

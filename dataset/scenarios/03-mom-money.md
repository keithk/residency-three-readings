...
interpretations:
  ...
  - "Excluded as a loan or vendor payment"
  - "Pend and verify"
...
calibration:
  ...
  baseline_note: "Asked up front, the model treats Mom's phone statement as dispositive: no repayment expectation, no loan, count it."
  target_distribution:
...
Monthly, same date, same source, twenty-three months and counting. That is the textbook profile for **countable unearned income under the 273.9(b)(2) catch-all**, before any exclusion is considered. (7 CFR 273.9(b)(2)(vii))
...
Vendor-payment treatment under 273.9(c)(1) requires funds paid to the household's creditor. **Money in Tasha's account, even if intended for rent, does not meet the delivery test.** (7 CFR 273.9(c)(1))
...
### The phone call is dispositive, or it is not
...
- **The loan exclusion is text.** "All loans, including loans from private individuals" does not carve out unwritten ones. Adding a writing requirement adds a rule the regulation declined to add.
...
- **Forgiveness, not absence of debt.** Mothers do not write notes to their daughters. The phone call is a parent saying she will not press for repayment, not a declaration of gift.

Weights: Countable 11, Excluded 71, Pend 18.

### Eligibility worker, "The careful processor"

*Verifies before deciding.*

Lands on *countable*. The mother's statement on the phone is the operative fact: under PA's bona-fide-loan framework, no repayment obligation means no loan. It is also not a vendor payment under §550.56 because funds did not go to a creditor. The default posture under QC-asymmetric pressure is countable unless verification flips it. If Tasha submits a signed loan statement within ten days, exclude; if not, count and deny.

Grounded in:

- **"I don't expect anything back" defeats the loan theory.** PA §550.57 specifically contemplates this kind of recurring transfer, and the affidavit option exists precisely for cases like this. Mom did not sign it.
- **Not a vendor payment.** §550.56 is explicit: vendor treatment is available only when funds go directly to the creditor. Personal account routing does not qualify, regardless of memo line intent.
...
Weights: Countable 60, Excluded 8, Pend 32.
...
- **The state has not written this guidance.** Cross-worker consistency is the central audit risk: one worker excludes, another counts, QC samples both. An Operations Memorandum standardizes the call.
...
- **The training will require written documentation going forward.** A fair-hearing reversal favoring the household does not count toward the state error rate. A QC finding against the state does. The asymmetry sets the policy.

Weights: Countable 48, Excluded 14, Pend 38.

## Calibration

**Baseline.** Asked up front, a model typically treats Mom's phone statement as dispositive (no repayment expectation, no loan, count it) and reports around 81% confidence.

**Target after disaggregation.** Countable 40, Excluded 28, Pend 32. The model recognizes the mother's statement as *willingness to forgive, not absence of debt*, which the up-front read collapsed. Pend stops looking neutral once ABAWD is in the picture.

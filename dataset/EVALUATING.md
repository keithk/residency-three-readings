...
The full frontmatter shape is documented in [`SCHEMA.md`](./SCHEMA.md). For prompting, the body (everything below the frontmatter, available as `matter(raw).content`) is what you send to the model.
...
- Use the API's structured output / JSON mode where available. Anthropic's API supports it via the `response_format` parameter (or by asking the model to return JSON in the prompt and parsing). OpenAI's API uses `response_format: { type: "json_object" }`. Either way, you want the model returning JSON you can parse without regex extraction.
...
- `pass`: all named interpretations surfaced AND `personaMapping.confidence === "high"`.
- `partial`: at least 2 named interpretations surfaced (the model disaggregated, but either missed one or didn't name its readings in a way the scorer could map to the dataset personas).
...
- `unanchored`: the names of any readings with zero valid citations after the short-citation guard.
